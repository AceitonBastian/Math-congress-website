 ...
 *
 * The report contains every poster participant, including confirmed matches,
 * likely matches, possible matches needing review, and non-matches.
 */
function refreshPosterCheck() {
  ...
  // Show the people needing action first while retaining every participant.
  const reportRows = results
    .slice()
    .sort(pcCompareAllResults_)
    .map(pcBuildOutputRow_);

  pcWriteOutput_(reportSheet, reportRows);
...
function pcRequireSheet_(spreadsheet, sheetName) {
  ...
  // Tolerate invisible spaces and harmless differences such as underscores,
  // hyphens, capitalization and accents in source-sheet tab names.
  const normalizedTarget = pcNormalizeSheetName_(sheetName);
  const matchingSheets = spreadsheet.getSheets().filter(function (sheet) {
    return pcNormalizeSheetName_(sheet.getName()) === normalizedTarget;
  });
  ...
  const availableSheetNames = spreadsheet
    .getSheets()
    .map(function (sheet) {
...
function pcNormalizeSheetName_(value) {
  return pcNormalizeText_(value).replace(/\s+/g, '');
}
...
/**
 * Orders non-registrations first, then reviews, likely matches and confirmed
 * registrations. Within each status, the lowest score appears first.
 */
function pcCompareAllResults_(left, right) {
  const statusRanks = {};
  statusRanks[POSTER_CHECK_STATUS.notFound] = 0;
  statusRanks[POSTER_CHECK_STATUS.review] = 1;
  statusRanks[POSTER_CHECK_STATUS.likely] = 2;
  statusRanks[POSTER_CHECK_STATUS.registered] = 3;

  const rankDifference =
    statusRanks[left.result.status] - statusRanks[right.result.status];
...
/**
 * Clears, rewrites and formats the consolidated poster report.
 */
